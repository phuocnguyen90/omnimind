...
export class ZoteroDB {
  ...
  /**
   * Retrieves single parent item metadata and file path details from Zotero database by its key.
   */
  public async getItemPayloadByKey(key: string): Promise<any | null> {
    ...
    // Resolve Zotero dynamic field IDs
    const fieldQuery = `SELECT fieldName, fieldID FROM fields WHERE fieldName IN ('title','date','abstractNote')`;
    const fStmt = db.prepare(fieldQuery);
    const fids: Record<string, number> = {};
    while (fStmt.step()) {
    ...
    const fileName = result.file_name ? (result.file_name as string).replace('storage:', '') : '';
    
    // Format the Title as a BibTeX-style citation string
    let richTitle = fileName;
    if (result.title) {
      const yearMatch = result.year ? String(result.year).substring(0,4) : "n.d.";
      let authorStr = "Unknown";
      if (result.authors) {
         const authorsList = String(result.authors).split(';');
         if (authorsList.length === 1) {
           authorStr = authorsList[0].split(',')[0].trim();
         } else if (authorsList.length === 2) {
           authorStr = `${authorsList[0].split(',')[0].trim()} & ${authorsList[1].split(',')[0].trim()}`;
      ...
      richTitle = `${authorStr} (${yearMatch}) - ${result.title}`;
    }

    return { ...result, rich_title: richTitle };
  }
}
