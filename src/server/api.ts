...
import { lmClient, embedder } from "../index";
import { reembedStatus, runReembedding } from "../ingestion/reembed";
    ...
    try {
      ...
      if (req.url?.startsWith('/api/')) {
        if (req.method === 'POST' && req.url === '/api/pause') {
          ...
          let modelStatus = { mismatch: false, savedModel: '', activeModel: '', error: '' };
          try {
            if (embedder) {
              const activeModel = await embedder.resolveEmbeddingModel();
              modelStatus.activeModel = activeModel ? activeModel.identifier : 'None';
              
              if (vectorStore) {
                const isMismatch = await vectorStore.checkModelMismatch(modelStatus.activeModel);
                if (isMismatch) {
                  modelStatus.mismatch = true;
                  const dbModel = await vectorStore.getDatabaseModel();
                  modelStatus.savedModel = dbModel || 'Unknown';
                  modelStatus.error = `Database contains chunks generated by '${modelStatus.savedModel}', but the active model is '${modelStatus.activeModel}'.`;
                }
              }
            }
          } catch (e: any) {
            modelStatus.mismatch = true;
            modelStatus.error = e.message || String(e);
          }

          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ 
            state: jobQueue.getState(), 
        ...
        } else if (req.method === 'POST' && req.url === '/api/knowledge/reembed') {
          jobQueue.pause();
          runReembedding().then(() => {
            console.log("[API] Background re-embedding completed.");
          }).catch(err => {
            console.error("[API] Error in background re-embedding:", err);
          });
          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ success: true, message: "Re-embedding process started." }));
        } else if (req.method === 'GET' && req.url === '/api/knowledge/reembed/status') {
          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify(reembedStatus));
